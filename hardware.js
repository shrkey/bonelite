...
function setPath( pin ) {
	
	//console.log('Finding pin : ' + pin);
    try {
        var ocp = file.file_find('/sys/devices', 'ocp.');
		//console.log( ocp );
		
        var pwm_test = file.file_find(ocp, pin + '.', 10000);
		
		//console.log( pwm_test );
		
        file.file_find(pwm_test, 'period', 10000);
        pwmPath[pin] = pwm_test;
        
    } catch(ex) {
        console.log('Error finding PWM controls: ' + pin);
    }
...
exports.startPWM = function( pin ) {
	...
	if( pwmPath[pin] == undefined ) {
		// We don't have a path so build one
		setPath( pin );
	}
	
    var path = pwmPath[pin];
...
exports.writePWMFreqAndValue = function(pin, pwm, freq, value, resp) {
    ...
    try {
        ...
        console.log('error updating PWM freq and value: ' + path + ', ' + ex);
    }
    return true;
};
